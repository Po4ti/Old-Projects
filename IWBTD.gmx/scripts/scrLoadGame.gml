...
if (loadFile)
{
    ...
    if (saveMap != -1)  //check if the save map loaded correctly
    {
        ...
        global.pr = ds_map_find_value(saveMap,"pr");  
        global.pb = ds_map_find_value(saveMap,"pb");        
        for (var i = 0; i < 26; i++)
        {
            global.at[i] = ds_map_find_value(saveMap,"at["+string(i)+"]");
        }
        for (var i = 0; i < 26; i++)
        {
            global.plx[i] = ds_map_find_value(saveMap,"plx["+string(i)+"]");
        }
        for (var i = 0; i < 26; i++)
        {
            global.ply[i] = ds_map_find_value(saveMap,"ply["+string(i)+"]");
        }        
